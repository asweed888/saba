...
func (u *declareUseCase) SubAction() error {
    decl, _ := u.DeclareRepository.Load()
    err := decl.TacitSetting.SubAction(decl)
    if err != nil {
...
func (u *declareUseCase) GenerateArch() error {
    ...
    for _, s := range decl.Spec {
        ...
        workdir := s.ChangeDirectory(conf.RootPath(decl))
        if err := s.CreateDirectory(workdir); err != nil {
            return utils.Error(err)
        }

        if len(s.Upstream) != 0 {
            if err := generateUpstream(workdir, s.Upstream, conf); err != nil {
                return utils.Error(err)
            }
        }

        if len(s.CodeFile) != 0 {
            if err := generateCodeFile(workdir, s.CodeFile, conf); err != nil {
                return utils.Error(err)
            }
        }
    }
...
func generateUpstream(prevWorkDir string, upstream []*model.DeclareUpstream, conf *model.TacitSetting) error {
    ...
    for _, ups := range upstream {
        ...
        if err := ups.CreateDirectory(workdir); err != nil {
            return utils.Error(err)
        }

        if len(ups.Upstream) != 0 {
            if err := generateUpstream(workdir, ups.Upstream, conf); err != nil {
                return utils.Error(err)
            }
        }

        if len(ups.CodeFile) != 0 {
            if err := generateCodeFile(workdir, ups.CodeFile, conf); err != nil {
                return utils.Error(err)
            }
        }
    }
